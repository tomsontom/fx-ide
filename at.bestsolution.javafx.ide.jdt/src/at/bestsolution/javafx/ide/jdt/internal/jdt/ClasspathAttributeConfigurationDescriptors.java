...
public class ClasspathAttributeConfigurationDescriptors {
	...
	private static class Descriptor {
		...
		public Descriptor(IConfigurationElement configElement) throws CoreException {
			...
			if (name == null) {
				throw new CoreException(new Status(IStatus.ERROR, JavaPlugin.ID_PLUGIN, 0, "Invalid extension (missing attributeName)", null)); //$NON-NLS-1$
			}
			if (pageClassName == null) {
				throw new CoreException(new Status(IStatus.ERROR, JavaPlugin.ID_PLUGIN, 0, "Invalid extension (missing class name): " + name, null)); //$NON-NLS-1$
			}
		}
		...
		public ClasspathAttributeConfiguration getInstance() throws CoreException  {
			if (fInstance == null) {
				...
				if (elem instanceof ClasspathAttributeConfiguration) {
					...
					throw new CoreException(new Status(IStatus.ERROR, JavaPlugin.ID_PLUGIN, 0, "Invalid extension (page not of type IClasspathContainerPage): " + getKey(), null)); //$NON-NLS-1$
				}
			}
			return fInstance;
		}
	...
	private static HashMap<String, Descriptor> readExtensions() {
		IConfigurationElement[] elements = Platform.getExtensionRegistry().getConfigurationElementsFor(JavaPlugin.ID_PLUGIN, ATT_EXTENSION);
